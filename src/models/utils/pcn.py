    ...
    for epoch in range(501):
        ...
        with tqdm(trainDataLoader, desc=f"Epoch {epoch+1}/501", unit="batch", total=iters_per_epoch) as tepoch:
            for i, data in enumerate(tepoch):   
                ...
                c =  c[:,:3,:].transpose(1,2).contiguous().to(opt.device).float()


                # # caseB:
                # c = data
                # c = c[:,:,:3].to(opt.device).float()
                
                epoch_iter += len(c[0])  
                pp= fps_gs(c, 64,attribute=['xyz'], random_start=True)             
                optimizer.zero_grad()

                # forward propagation
                
                coarse_pred, dense_pred = model(pp)
                print(coarse_pred.shape, dense_pred.shape, c.transpose(1,2).shape)
                # loss function

                loss1 = cd_loss_L1(coarse_pred.transpose(1,2), c.transpose(1,2))
        
                loss2 = cd_loss_L1(dense_pred.transpose(1,2), c.transpose(1,2))
        ...
        if epoch >0 and epoch % 10 == 0:   # 每10个epoch显示一次
            ...
            coarse_pc = coarse_pred[0].detach().cpu().numpy() # coarse 1024 点
            dense_pc  = dense_pred[0].detach().cpu().numpy()  # dense 重建点
